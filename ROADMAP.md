...
- **第 2 週（~07-27）**：C+D 混進同一次 SFT 重練 ✅（3869129/3869130，07-19 完成）→
  ...
  驗證 jobs 3875151/3875152 ✅ 完整跑完 → C ✅ 有效且無副作用、D ✅ 有效——**且 D 在 qa-context
  的 360 列上更乾淨（99% vs 18%），證據已從 100 列擴到 460 列**（3876525/3876526 ✅）；
  代價來自 bho pack 稀釋 → **C-only adapter 3876434 ✅ 已練好**（train_loss 0.9648 ≈ plain 的
  0.9656），驗證 **3878452 / 3878453** 跑步中 → 明早三選一，見「接下來」#1）→ E ✅（路由在 v2 重驗存活）+ F 🟡（runaway 守門 ✅、
  GlotLID 待做）
...
1. 🟡 **primary 配方：驗證跑完了，答案是「C 留、D 留、但 C+D 綁在同一顆 adapter 不該當
   ...
   - **D 也該留，07-21 起覆蓋率從 100 列擴到 460 列**：錯語言的輸出 chrF 近乎零分，bho 那一欄
     的拉抬跟 OEG 整體 −1.67 是**同一個數量級**（bho 是 24 語之一）——這正是 dev 的 −1.42
     定不了案的原因。qa-oeg 100 列：**40% vs 12%**；qa-context 360 列（佔 bho 的 78%）：
     **99% vs 18%**（contrastive function words，jobs 3876525/3876526）——**後者更乾淨**。
     ⚠️ **別把「460 列」講成「460 列的證據」**（本文件先前這樣寫過，已改）：460 列是**生成覆蓋率**
     （兩個 adapter 都 460/460），**實際給出語言判定的只有約一半——C+D 241 列、plain 193 列**。
     其餘分兩種，都不算失敗：**拒答**（71/62 列，輸出是實證固定字串 `जवाब नइखे`，照定義就是 bho
     但那是背下來的片語，對「會不會自己生成 bho」零資訊）與**太短**（125/184 列，2–3 詞名詞片語，
     多半直接抄自段落，原理上沒有足夠文字構成一個語言）。
   - **→ 下一步（擋著 07-27 定案的唯一一件事）**：練 **C-only**——
     ...
     SFT job **3876434 ✅ 完成（6h27，07-21）**（`--data data/train_v2-c.jsonl --no-lang-hint`）。
     → **兩個驗證 job 已送出**：**3878452**（`lora_eval`，dev_v2 2,949 列，~5h）
     ＋ **3878453**（`run_test.py --task qa-oeg`，2,359 列，~13h）→ `verify_outputs.py`。
     **兩個都要**——dev 量不到 compliance、測試集算不出品質分數，這正是本輪的起點。
   - 🟢 **train_loss 已先透露一個訊號：C-only 0.9648 vs plain 0.9656 vs C+D 1.156。**
     同樣的 11,674 個樣本，C-only 和 plain 差 **0.0008**（實質相同）→ **那 840 句字數預算
     幾乎不增加擬合難度，C+D 的 loss 上升完全是 bho pack 造成的**（續寫／翻譯文本，佔 41%）。
     ⚠️ 但**別把它當答案**：train_loss 是在訓練分佈上算的，dev 是另一個分佈——C 完全可能
     不增加擬合難度、卻仍改變生成行為。要等 3878452。
   ...
   - **決策樹**：
     ...
     (b) 3876434 的 dev **也掉** → pack 是無辜的，−1.42 來自 C 本身或訓練噪音 → C+D 直接當
         primary，small 版不用送。
   - **份量粗算**（給 (a) 用，07-21 上修）：錯語言 chrF 近乎零分，bho 那欄從 ~10 拉到 ~25、
     攤到 4.2% 約 **+0.6**，對上 −1.42 → **純看自動指標仍是虧的，但差距比原估小**，因為
     qa-context 那 360 列的效果（99% vs 18%）比當初拿來估的 qa-oeg（40% vs 12%）大得多，
     而它們佔 bho 的 78%。加上人工評審那關「整段答錯語言」是最刺眼的失敗模式、自動指標嚴重低估
     → **D 要留，但要留得成比例**，這正是 C+D-small 的用意。
...
4. 🟡 **bho 的 qa-context 缺口——460 列裡有 360 列從沒測過（07-21 開跑）**。
   到目前為止所有 D 的證據都來自 qa-oeg 那 **100** 列；qa-context 的 **360** 列零資料。
   讀完那 360 列後發現**失敗模式和量測工具都不一樣**：
   - **97% 是跨語言**（段落語言：eng 100、arb/spa/zho 各 25、fra/rus 各 20…，只有 10 列的段落是
     bho）→ 多一種 qa-oeg 沒有的失敗：**直接用段落的語言作答**，不只是漂成印地語。
   - 🔴 **prompt 要求「एके वाक्य में जवाब दे सकीं」＝一句話**，答案約 10–25 詞，而 `bho_lid.py`
     自己的 docstring 寫明「usable on anything paragraph-sized, **not to be trusted on single
     short sentences**」→ **主力工具在這 360 列上無效**，`verify_outputs.py` 現在會**跳過**
     qa-context 的 bho_lid，而不是照跑然後假裝可信。
   - 換上三個在單句上仍成立的檢查（已實作並用合成資料自測，六個數字精確重現）：
     **① 書寫系統**（Unicode range，100% 抓到用英/中/阿拉伯文作答，但分不出 bho/hin——同為天城文）、
     **② 拒答率**（對 `constraint_bank` 的實證字串 `जवाब नइखे` 精確比對）、
     **③ 一句話遵循**（數句尾符號）。並列印「錯誤書寫系統 × 段落語言」的交叉表，直接看漂移是不是
     被段落拉走的。
   - **bho vs hin 這一格故意留白**：單句上沒有可信的判法 → 交給下面的 GlotLID，或人工讀 40 例。
     誠實一點說，短抽取式答案有一部分是段落搬來的專有名詞，「屬於哪個語言」**原理上不可判定**；
     反過來看，這也表示 360 列的漂移風險結構上**低於** 150 詞的自由生成。
   - **跑起來很便宜**：`run_test.py` 本來就有 `--lang`，不必改任何提交路徑的程式。
     → ✅ **jobs 3876525（C+D 3869129）/ 3876526（plain 3867139）已完成（07-21，35min/30min，
     比估的 2h 快得多——因為答案只有一句話，token 少）**。
   - 🟢 **結果：D 在 qa-context 上有效，而且比 qa-oeg 那 100 列顯示的更乾淨——但差點被自己的
     聚合數字騙過去。**
     - ①②③ 三個檢查說**兩個系統完全一樣**：天城文 358/360（兩邊都是）、一句話 ~99%（兩邊都是）、
       拒答 19.7% vs 17.2%。照這個讀法，結論會是「D 在這 360 列沒作用」。
     - **人工對讀推翻了它**：C+D 寫 `आ`／`खातिर`／`होखल`，plain 寫 `और`／`के लिए`／`की`。
       是**聚合瞎了，不是效果不存在**。
     - → 補上第四個檢查 **contrastive function words**（問「這兩個字選了哪個」，一個詞就夠判，
       不像 `bho_lid` 的 marker **密度**需要一整段）：
       **C+D 99% bho（163:1）vs plain 18% bho（21:93）**，各有 125/184 列因為太短而不可判定。
       單看 `और`／`आ` 一組：C+D 用 आ 82 次、用 और **0** 次；plain 用 आ 5 次、用 और **67** 次。
     - **意義**：D 的價值現在有 **460 列**的證據，不是 100 列，而且 qa-context 這 360 列
       （佔 bho 的 78%）的效果**比 qa-oeg 更明確**。→ **primary 帶 D 的理由變強了**，
       #1 決策樹裡「C-only 就算 dev 漂亮也不該直接選」這一條更站得住。
     - ⚠️ 仍未答：可判定的只有 39%，其餘是 2–3 詞的名詞片語（中位 **3 詞**），
       語言歸屬原理上不可判定；且這只證明「像博傑普爾語」，沒有證明**答得對**（測試集無標準答案）。
5. **F：GlotLID 全語言輸出守門**（`cis-lmu/glotlid`，有 bho_Deva）＋ 錯語言重生成；
   best-of-N 仍未動工，時間不夠就只上語言守門。**新增一個非做不可的理由**：上面 #4 的
   bho-vs-hin 那一格只有 GlotLID 補得起來（訓練過的分類器，短文本可用），`bho_lid` 的功能詞
   密度法在單句上結構性失效。⚠️ 計算節點無網路 → 模型要先在 login node 預先快取。
6. **G：最後衝刺排程**（截止 08-01 AoE，倒數兩週）：
   ...
   - Google Form 提交。
   - 🟢 **排程警告已下修（07-21 修正，原估 67h 是錯的）**：先前只量了 qa-oeg（20.5 s/列）就外推到
     全部，但 **qa-context 只要 5.9 s/列**（jobs 3876525/3876526 實測）——因為它只要一句話，
     token 少得多，而 qa-context 佔 10,999 列裡的 **8,640 列（79%）**。
     重算：8,640×5.9 + 2,359×20.5 = **27.5 小時**，不是 67。
     → 仍超過 24h 上限，但 **`--shard i/2` 每片 13.8h 就夠**（n=3 則 9.2h），不必切 4–6 片。
     ⚠️ 5.9 s/列是在 bho 上量的；「一句話」的指示每個語言都有，所以應該接近，但正式跑前值得再抽驗。
     細節記在 IMPLEMENTATION_NOTES §6。
7. **兩個輸出瑕疵，已在完整 2,359 列上量化**（兩個 adapter 都有，所以都不是 C 或 D 造成的；
   ...
   **這是整張 roadmap 上最便宜的一分**：交卷前一行後處理清掉即可，該在 G 階段的提交腳本裡做掉。
   ...
   除非 F 的語言守門順手一起做。
8. **小工具債**：`evaluate.py` 自動輸出 qa-oeg 加權聚合（0.87/0.13）＋ truncated 拒答指標，
   免得每次手算；EM/F1 的 TASK_PROXY 混池問題（待辦 #6）在 v2 時代已無關緊要，不修。
9. **傳話給隊友**：test sum 輸入 p50 2,773 詞超過 `train_lora.py` 的 2048-token 截斷；
