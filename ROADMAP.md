...
4. 🟡 **bho 的 qa-context 缺口——460 列裡有 360 列從沒測過（07-21 開跑）**。
   ...
   - **跑起來很便宜**：`run_test.py` 本來就有 `--lang`，360 列 × 20.5 s ≈ **2h/顆**，不必改任何
     提交路徑的程式。→ **jobs 3876525（C+D 3869129）/ 3876526（plain 3867139）已送出（07-21）**。
     這個對照比 qa-oeg 那次**更有力**：360 列 vs 100 列。
