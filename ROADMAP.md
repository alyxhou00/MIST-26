...
| G | **三份提交對沖** | primary = 蒸餾+路由完全體；variant1 = 9B 3-shot（27.64 安全牌）；variant2 = 激進版（best-of-N） | ⬜ 策略微調後照走（蒸餾出局 → primary = **gold-v2 adapter 路由**，C+D 或 plain 版看 **3875151/3875152** 的完整結果，見「接下來」#1）；具體排程見「接下來做什麼」#5。variant1 = 3-shot 安全牌在 v2 dev 的新基準是 37.25/57.37/45.67/34.47 |
...
- **第 2 週（~07-27）**：C+D 混進同一次 SFT 重練 ✅（3869129/3869130，07-19 完成）→
  🟡 **primary 二選一還沒定案**（C+D 在 dev 輸 1.42 qa-oeg，但 dev 對 C 和 D 都是盲的 ——
  驗證 jobs 3875151/3875152 跑步中）→ E ✅（路由在 v2 重驗存活）+ F 🟡（runaway 守門 ✅、
  GlotLID 待做）
...
1. 🟡 **primary 二選一：C+D adapter（3869129）vs plain-v2 adapter（3867139）**——這是現在唯一
   擋在最終配方前面的決定。已知的兩半證據互相矛盾，所以**不要只看 dev**：
   - **dev（3869130 vs 3869088）：C+D 輸**。belebele 44.70/44.33、tydiqa 70.72/72.04、
     MCIF 51.11/50.95（qa-context 三欄都在 ±0.4 噪音內），**qa-oeg agg 38.57 vs 39.99（−1.42）**。
   - **但 dev 對 C 和 D 兩者都是盲的**（已查證，不是推測）：`dev_v2` 的 qa-oeg prompt 帶字數預算的
     有 **0 列**、bho **0 列**。dev 只量得到 C+D 的**代價**（bho pack 佔訓練混合 41% 稀釋 qa 頭、
     train_loss 0.9656→1.156），量不到它的**好處**。
   - **好處要在官方測試 prompt 上量**：jobs **3875151**（C+D）/ **3875152**（plain 對照），
     `run_test.py --task qa-oeg`（2,359 列，含 465 列預算 + 100 列 bho），
     `python scripts/verify_outputs.py <兩個輸出檔>` 計分。
   - **前 517 列初步（❗ 語言是按順序生成的，只有 arb/ben/bho/ces/ckb + 部分 deu，C 的數字還不具
     代表性；bho 那 100 列已全跑完）**：compliance **57.0% vs 34.6%**、bho **40% vs 12%**。
   - **決策規則**：等 2,359 列跑完（24 語全到齊）再定。若 compliance 與 bho 的優勢維持，
     用 −1.42 的 qa-oeg dev 差換 460 列 bho + 465 列預算遵循是划算的（且人工評審偏好遵守指令）；
     若優勢在完整語言集上縮水，就選 plain-v2。
...
6. **兩個在驗證時撞見、還沒處理的輸出瑕疵**（EXPERIMENTS_NEW.md 有細節）：
   (a) **退化重複**——bho 輸出會把同一句重複 8–10 次，兩個 adapter 都有；不是 runaway
   （沒有假對話輪），現有的 stop-fix 攔不到，要用 `repetition_penalty` 或 n-gram 阻擋才治得了。
   (b) **字面 `<br><br>` markup**混在預測裡，plain-v2 也有 → 來自 qa 語料（網頁來源的 aya/oeg），
   不是 bho pack；交卷前做一次後處理清掉就好，成本近乎零。
